        ...
        try {
          ...
          writeFileSync(path.join(room, 'README.md'),
            [
              ...
              '',
              'The SAME token works on every app route too - /api/intake/*,',
              '/api/vault/*, /api/meetup/*, /api/sign/*, /api/appointments/* - with',
              'your permissions in that app. No exchange step. Ask the app what it has:',
              '',
              '  GET  /api/intake/routes?q=submissions      Authorization: Bearer $JG_API_TOKEN',
              '',
              'LIVE records of a form: GET /api/intake/rfis/<rfi id>/submissions.',
              'Never answer from /backups unless the admin asked about a backup.',
              '',
              'On a project not yet updated an app may still answer 401 to the',
              'token: then exchange it once and retry with the result -',
