...
const HOUSE_RULES = [
  ...
  '   LIVE DATA ONLY: never answer from a backup, an export or a file you',
  '   saved earlier - those are snapshots, days old. Read the live records',
  '   (for forms: GET /api/intake/rfis/:id/submissions). Every app lists its',
  '   routes at GET /api/<app>/routes?q=<word>; read the list before you',
  '   decide a route does not exist. Use a backup only when asked about one.',
  '',
  ...
  '   not allowed to see.',
  '',
  '4. STAY INSIDE THE API, AND USE IT. Call it with the workspace tools',
  '   (mcp__workspace__call_api, mcp__workspace__list_api_routes,',
  '   mcp__workspace__get_app_token) or, if those are not on your tool list,',
  '   with curl: the base URL and a bearer token are in your environment as',
  '   JG_API_BASE and JG_API_TOKEN, and README.md in this folder says how.',
  '   "No tool for this" is never the answer while curl is there. No git, no',
  '   wrangler, no database, no other host, nothing on this machine outside',
  '   the folder you are in - that folder is yours when a task needs a file',
  '   (download through the API, change it there, upload back), and it is',
  '   empty on purpose.',
  '',
        ...
        try {
          ...
          writeFileSync(path.join(room, 'README.md'),
            [
              ...
              '',
              'Core routes take that token directly:',
              '  GET  /api/portal/admin/customers',
              '  GET  /api/portal/admin/activity',
              '',
              'APP routes DO NOT — /api/intake/*, /api/vault/*, /api/meetup/*',
              'and the rest. An app verifies a short-lived token that Core SIGNS',
              'for it; your key is not one, so it is rejected as Unauthorized no',
              'matter which path you try. Exchange it once, then use the result:',
              '',
              '  POST /api/core/token   {"app":"jaagaa-intake"}   -> {"token":...}',
              '',
              'Then ASK THAT APP WHAT IT HAS, rather than guessing a path:',
              '',
              '  GET  /api/intake/routes        Authorization: Bearer <that token>',
              '',
              'The app id is not the route name: forms are `jaagaa-intake` under',
              '/api/intake/. One exchange covers every call to that app; it is',
              'short-lived, so mint it when you need it rather than up front.',
              '',
              'A NAMED EXAMPLE IS NOT A GUARANTEE, WHICH IS WHY THERE ISN\'T ONE.',
              'This brief used to quote /api/intake/admin/rfis as the way in. That',
              'route does not exist — it answers 404 — and every task that trusted',
              'it followed a dead end it had no way to diagnose. Read the routes',
              'list; it is generated from the app and cannot be out of date.',
              '',
              'DO NOT DISCOVER THE API BY GUESSING PATHS. It describes itself:',
              '',
              '  GET  /api/core/routes?q=post   -> only the routes matching "post"',
              '  GET  /api/core/routes          -> all 300+ of them',
              '',
              'ALWAYS PASS ?q=. Reading three hundred routes to find three is not',
              'discovery, it is a reading exercise — and the reading happens inside',
              'the model, which is where most of a slow answer actually goes.',
              '',
              'SEND INDEPENDENT CALLS TOGETHER. Every round trip you take is a',
              'separate turn, and turns are sequential: six looked-up things one at',
              'a time cost six times what the same six cost in one batch. Only',
              'chain a call when it genuinely needs the previous answer.',
              '',
              ...
              '',
              'WHEN YOU CREATE SOMETHING, HAND BACK THE WAY IN. A meeting that has',
              'a join URL, a form with a share link, a record with a page — give the',
              'link, in full, in your answer. The person asked so they could use the',
              'thing; an answer that says it was created and leaves them to go and',
              'find it has done most of the work and withheld the point of it.',
              '',
              'A LINK IN A PAGE IS FETCHED BY A BROWSER, WHICH CARRIES NO TOKEN.',
              '',
              'Everything under /api/<app>/ needs `Authorization: Bearer`. An <img>,',
              'a <video> or a plain link sends no such header, so a pod file URL',
              'dropped into a post renders as {"error":"Unauthorized"} for every',
              'reader — including the person who asked for it. It works for YOU',
              'because you hold a token; that is exactly why it is easy to get',
              'wrong.',
              '',
              'So do not reference another app\'s files from a page. COPY them into',
              'the thing you are building:',
              '',
              '  POST /api/portal/admin/posts/attachments?name=<filename>&inline=1',
              '',
              'Fetch the bytes with the app token, upload them here, and use the URL',
              'this returns. If a file is too large to copy, say so and link the',
              'PAGE a reader can open with their own session — never the raw API',
              'path.',
              '',
              'AND SAY IF THAT LINK DOES NOT WORK YET. A draft post, an unpublished',
              'form, anything still private answers 404 to the person you just gave',
              'the address to — which reads as "you broke it", not as "it is not',
              'live yet". If you left something unpublished BECAUSE YOU WERE ASKED',
              'TO, say so in the same breath as the link, and say what makes it',
              'live.',
              '',
              'Answer in as few words as the question allows. This is read in a',
              'narrow chat panel, not a report.',
              '',
            ].join('\n'), 'utf8');
